...
public class PhoneStateSampleActivity extends Activity {
	...
	private List<LinkAddress> Addrlist = new ArrayList<LinkAddress>();
	...
	private void displayTelephonyInfo() {
         ...
         *
         ***************/
////		final List<CellInfo> allCellInfo = tm.getAllCellInfo();
////		for(CellInfo cellInfo : allCellInfo){
////		    if (cellInfo instanceof CellIdentityLte) {
////                CellIdentityLte cidlte = ((CellInfoLte) cellInfo).getCellIdentity();
////                String mccGetString = cidlte.getMccString();
////                String mncGetString = cidlte.getMncString();
////                String mobileNwOp = cidlte.getMobileNetworkOperator();
////            }
////        }
//
//        String cidLteMcc = null;
//        String cidLteMnc = null;
//        int cidLteTac = 0;
//        int cidLtePci = 0;
//        int cidLteBW = 0;
//
...
//                cidLteMcc = cellIdentity.getMccString();
//                cidLteMnc = cellIdentity.getMncString();
//                cidLteBW = cellIdentity.getBandwidth();
//                cidLtePci = cellIdentity.getPci();
//                cidLteTac = cellIdentity.getTac();
...
//        }
		 ...
		 * 2018 07 26 UPDATE
		 *
		 * Internet PDN IP Version Check
		 *
		 * use : ConnectivityManager
		 * TODO : 위에 List 부분 수정 필요. 지금 실행하면 Null 로 체크.
		 */
		...
		try{
			...
			lk = cm.getLinkProperties(network);
			Addrlist = lk.getLinkAddresses();
		}
		catch (Exception e){
			e.printStackTrace();
		}
		...
		for(int i=0 ; i<Addrlist.size() ; i++){
			if(Addrlist.get(i).toString().contains(".")){
				ipversion = "IPv4";
				break;
			}
			else if(Addrlist.get(i).toString().contains(":")){
				ipversion = "IPv6";
				break;
			}
		}
		...
		String simoperator_get = tm.getSimOperator();
		String networkoperator_get = tm.getNetworkOperator();
...
//		deviceinfo += ("Phone Type: " + phonetype + "\n");
		deviceinfo += ("================\n");
		deviceinfo += ("getSimOperator : " + simoperator_get + "\n");
		deviceinfo += ("getNetworkOperator : " + networkoperator_get + "\n");
		deviceinfo += ("InternetPDN IPversion : " + ipversion + "\n");
//		deviceinfo += ("cidLTE_getMccString : " + cidLteMcc + "\n");
//		deviceinfo += ("cidLTE_getMncString : " + cidLteMnc + "\n");
//        deviceinfo += ("cidLTE_getBandwidth : " + cidLteBW + "\n");
//        deviceinfo += ("cidLTE_getPci : " + cidLtePci + "\n");
//        deviceinfo += ("cidLTE_getTac : " + cidLteTac + "\n");
